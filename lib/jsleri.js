...
exports.EOS = exports.THIS = exports.List = exports.noop = exports.version = void 0;
 ...
 */
var version = process.env.VERSION || ''; // dummy function which can be used as alternative for onEnter and onExit methods
...
var parse = function parse(element, str, tree, reKeywords) {
  ...
  var expecting = new Expecting(); // used to add a node to the tree
  ...
  var nodeRes = walk(element, 0, tree, element, true); // get rest if anything

  var rest = str.substring(nodeRes.pos).replace(RE_LEFT_WHITESPACE, ''); // set isValid to False if we have 'rest' left.

  if (nodeRes.isValid && rest) nodeRes.isValid = false; // add EndOfStatement to expecting if this is possible
  ...
  nodeRes.expecting = expecting.getExpecting(); // add expecting and correct pos to nodeRes if nodeRes is not valid

  if (!nodeRes.isValid) nodeRes.pos = expecting.pos; // return nodeRes
...
var List = function List(element, delimiter, _min, _max, opt) {
  ...
  this.max = _max === undefined || _max === null ? null : _max; // when true the list may end with a delimiter
