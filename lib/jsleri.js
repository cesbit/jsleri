...
var parse = function parse(element, str, tree, reKeywords) {
  ...
  var _walk = function walk(element, pos, tree, rule, isRequired) {
    ...
    if (element instanceof Choice) {
      ...
      for (i = 0, l = element.elements.length; i < l; i++) {
        children = [];
        nodeRes = _walk(element.elements[i], node.start, children, rule, true);
    ...
    if (element instanceof List) {
      pos = node.start;
      for (i = 0, l = 0;;) {
        nodeRes = _walk(element.element, pos, node.children, rule, i < element.min);
        if (!nodeRes.isValid) break;
        pos = nodeRes.pos;
        i++;
        nodeRes = _walk(element.delimiter, pos, node.children, rule, i < element.min);
        if (!nodeRes.isValid) break;
        pos = nodeRes.pos;
        l++;
      }
    ...
    if (element instanceof Optional) {
      nodeRes = _walk(element.element, node.start, node.children, rule, false);
    ...
    if (element instanceof Rule) {
      element._tested = {};
      element._tree = {};
      nodeRes = _walk(element.element, node.start, node.children, element, true);
      if (nodeRes.isValid) appendTree(tree, node, nodeRes.pos);
      return nodeRes;
    }
    ...
    if (element instanceof Sequence) {
      pos = node.start;
      for (i = 0, l = element.elements.length; i < l; i++) {
        nodeRes = _walk(element.elements[i], pos, node.children, rule, true);
     ...
     * This
     **************************************************************************/
    if (element instanceof _This) {
      if (rule._tested[node.start] === undefined) {
        rule._tested[node.start] = _walk(rule.element, node.start, node.children, rule, true);
  ...
  var nodeRes = _walk(element, 0, tree, element, true);
...
var List = function List(element, delimiter, _min, _max, opt) {
  ...
  _max = this.args[3];
  opt = this.args[4];
  if (!(element instanceof Jsleri)) throw '(Jsleri-->List) first argument must be an instance of Jsleri; got ' + _typeof(element);
  if (typeof delimiter !== 'string' && !(delimiter instanceof Jsleri)) throw '(Jsleri-->List) second argument must be a string or instance of Jsleri; got ' + _typeof(delimiter);
...
function Optional(element) {
  ...
  if (obj) return obj;
  element = this.args[0];
  if (!(element instanceof Jsleri)) throw '(Jsleri-->Optional) first argument must be an instance of Jsleri; got ' + _typeof(element);
  this.element = element;
}
...
function Repeat(element, _min, _max) {
  ...
  _min = this.args[1];
  _max = this.args[2];
  if (!(element instanceof Jsleri)) throw '(Jsleri-->Repeat) first argument must be an instance of Jsleri; got ' + _typeof(element);
  this.element = element;
  this.min = _min === undefined || _min === null ? 0 : _min;
  this.max = _max === undefined || _max === null ? null : _max;
}
...
var refSet = function refSet(element) {
  if (!(element instanceof Jsleri)) throw '(Jsleri-->Ref-->set) first argument must be an instance of Jsleri; got ' + _typeof(element);
  _extends(this, element);
};
function Ref(Cls) {
...
function Grammar(element, reKeywords) {
  ...
  if (grammar !== null) {
    Object.getOwnPropertyNames(grammar).forEach(function (name) {
...
function Rule(element) {
  ...
  if (obj) return obj;
  element = this.args[0];
  if (!(element instanceof Jsleri)) throw '(Jsleri-->Rule) first argument must be an instance of Jsleri; got ' + _typeof(element);
  this.element = element;
}
...
var _This = function This() {
  if (!(this instanceof _This)) return new _This();
};
_This.prototype = Object.create(Jsleri.prototype);
...
function Token(token) {
  ...
  if (obj) return obj;
  token = this.args[0];
  if (typeof token !== 'string') throw '(Jsleri-->Token) first argument must be a string; got ' + _typeof(token);
  this.token = token;
}
...
function Tokens(tokens) {
  ...
  if (obj) return obj;
  tokens = this.args[0];
  if (typeof tokens !== 'string') throw '(Jsleri-->Tokens) first argument must be a string; got ' + _typeof(tokens);
...
Jsleri.prototype.setCallbacks = function (args) {
  var first = args[0];
  if (first === undefined || first === null || typeof first === 'string' || first instanceof Jsleri || first.START !== undefined) return;
...
Jsleri.prototype.checkElements = function (a) {
  ...
  for (; i < l; i++) if (!(a[i] instanceof Jsleri)) {
    a[i] = new Token(a[i]);
  }
  return a;
};
...
export { version, noop, Keyword, Regex, Token, Tokens, Sequence, Choice, Repeat, List, Optional, Ref, Prio, THIS, Grammar, EOS };